    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get('type') == "approve":
            ...
            try:
                ...
                Profile.objects.filter(user=editor).update(reputation=F('reputation') + 5) #add 5 points to editor
                Profile.objects.filter(user=term.author).update(reputation=F('reputation') + 15) #add 15 points to author
